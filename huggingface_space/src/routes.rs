...
pub async fn register(
    ...
    let user = sqlx::query_as!(
        User,
        r#"
        INSERT INTO users (name, email, password_hash, role, bio, skills, linkedin)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, name, email, password_hash, role, bio, skills, linkedin, created_at
        "#,
        payload.name,
        payload.email.to_lowercase(),
        password_hash,
...
static RESET_CODES: std::sync::LazyLock<std::sync::Mutex<std::collections::HashMap<String, String>>> = 
...
pub async fn forgot_password(
    ...
    // Generate a 6-digit numeric code using UUID value modulo arithmetic
    let uuid_val = Uuid::new_v4().as_u128();
    let code = ((uuid_val % 900000) + 100000).to_string();

    // Store in global static map
    {
        let mut codes = RESET_CODES.lock().unwrap();
        codes.insert(email_lower.clone(), code.clone());
    }

    // Log the code to server console (which is visible in HF Spaces logs!)
    tracing::info!("🔑 [PASSWORD RESET] Reset code for {} is {}", email_lower, code);

    Ok(Json(ForgotPasswordRes {
        message: "Reset code generated. Check server logs or use the demo code below.".to_string(),
        code_for_demo: Some(code),
    }))
}
...
pub async fn reset_password(
    ...
    let code_valid = {
        let codes = RESET_CODES.lock().unwrap();
        if let Some(stored_code) = codes.get(&email_lower) {
    ...
    if !code_valid {
        return Err(AppError::BadRequest("Invalid or expired reset code".to_string()));
    }
